...
const WRIGHT_WEBHOOKS = {
  eliotMatch: N8N_WEBHOOKS.eliotMatch,
}
...
type MessageSender = 'eliot' | 'author' | 'ghost'
...
function WrightContent() {
  ...
  useEffect(() => {
    async function checkAccess() {
      ...
      addEliotMessage(
        "Hi, I'm Eliot. Before you meet your writing partner, I'd like to ask you a few questions — nothing complicated, just enough to understand you and your project. There are no wrong answers. This will take about two minutes."
      )
      setStage('welcome')
    }
    checkAccess()
  ...
  function addEliotMessage(text: string) { addMessage('eliot', text) }
  ...
  async function eliotSays(text: string, pauseBefore = 600) {
    setShowTyping(true)
    await delay(pauseBefore)
    setShowTyping(false)
    addEliotMessage(text)
  }
  ...
  async function handleQ1Submit() {
    ...
    await eliotSays('Thank you for sharing that.', 500)
    await eliotSays("Is this based on your own life, or is it a story you're telling?", 900)
    setStage('q2')
  }
  ...
  async function handleQ2Choice(choice: 'My own life' | "A story I'm telling" | 'Both') {
    ...
    await eliotSays(bridges[choice], 500)
    await eliotSays('Do you have anything written already — even rough notes or journal entries?', 900)
    setStage('q3')
  }
  ...
  async function handleQ3Choice(hasIt: boolean) {
    ...
    if (hasIt) {
      await eliotSays("That's a great start — more than you might think.", 500)
      setStage('q3-upload')
    } else {
      await eliotSays("Perfect. We'll build it together from scratch.", 500)
      await eliotSays('What does finishing this book mean to you?', 900)
      setStage('q4')
    }
  }
  ...
  async function handleFileSelect(file: File) {
    ...
    try {
      ...
      // Read text content for plain-text files so it can be sent to the match webhook
      if (ext === '.txt' || ext === '.md') {
      ...
      await eliotSays('What does finishing this book mean to you?', 800)
      setStage('q4')
    } catch {
      setIsUploading(false)
  ...
  async function handleQ4Submit() {
    ...
    await eliotSays('Thank you for telling me that.', 600)
    await eliotSays('One last question — would you prefer to work with a male or female writing partner?', 1000)
    setStage('q5')
  }
  ...
  const runEliotMatch = useCallback(async () => {
    const profile = authorProfileRef.current
    if (!profile) return

    let assignedAgent: GhostWriter = 'ivy'
    let eliotReflection = "That's a book worth writing."
    let bookTitle = ''
    let bookBrief: Record<string, unknown> = {}

    try {
      const res = await fetch(WRIGHT_WEBHOOKS.eliotMatch, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        // Webhook payload field names (rileyReflection / edenReflection) kept
        // as-is — n8n workflow rename lands in Wave 2. Frontend var name
        // updated to eliotReflection. Coordination brief 2026-07-30 covers
        // the payload rename lineage.
        eliotReflection = data.rileyReflection ?? data.edenReflection ?? eliotReflection
    ...
    // Show Eliot's reflection, then reveal
    await delay(1500)
    await eliotSays(eliotReflection, 300)
      ...
      case 'welcome':
        return (
          <div className="flex justify-center">
            <button
              onClick={() => { setStage('q1'); eliotSays("What's your book about?", 400) }}
      ...
      case 'q3-upload':
        return (
          <div className="space-y-3">
            ...
            <div className="flex justify-center">
              <button
                onClick={async () => { await eliotSays('What does finishing this book mean to you?', 600); setStage('q4') }}
      ...
      case 'done':
        return (
          <div className="flex justify-center py-2">
            <button
              onClick={() => router.push('/wright/studio')}
              className="px-8 py-3 rounded-xl bg-[#8FAF8A] text-white font-medium text-sm hover:bg-[#7a9e75] transition-colors"
            >
              Open your studio →
            </button>
    ...
    <div className="min-h-screen flex flex-col relative" style={{ backgroundColor: '#FAF8F4' }}>
      ...
      <div className="text-center pt-10 pb-4 flex-shrink-0">
        <div className="inline-flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-[#2C2C2C]">Eliot</span>
        </div>
      </div>
...
export default function WrightPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#FAF8F4' }}>
